...
```
AI  →  .\bridge\pw-bridge.bat <command>  →  bridge server (localhost:8080)  →  browser (CDP :9222)
```
...
- **If it fails:** Ask the user to open their browser in debug mode:
  > *"Please open the browser with remote debugging enabled (e.g., run `.\bridge\launch_brave_rp.bat` or start the browser with `--remote-debugging-port=9222`). Let me know when it's ready."*
...
After this, all `.\bridge\pw-bridge.bat` commands will route to the attached browser.
...
2. **Replace `playwright-cli` with `.\bridge\pw-bridge.bat`** in all commands when working through the bridge. The syntax is identical otherwise.
...
4. **All bridge commands run relative to the workspace root** using `.\bridge\pw-bridge.bat`.
